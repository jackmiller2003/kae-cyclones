...
class ExperimentCollection:
        ...
        for eigenLoss, eigenInits in self.runRegime.items():
            ...
            for eigenInit, stds in eigenInits.items():
                ...
                for std in stds:
                    ...
                    torch.save(min_model.state_dict(), f'/g/data/x77/{USER}/models/koopman/iclr_paper_models_22/{self.datasetName}-{eigenLoss}-{eigenInit}-{round(min_loss*1e2,2)}')
    ...
    def saveResults(self):
        with open(f"/home/156/{USER}/kae-cyclones/results/run_data/{self.name}.json", 'w') as f:
...
class Experiment:
    ...
    def run(self, epochs=50, batchSize=128, return_model=False, return_time=False, wd=0.01):
        ...
        eigvals, loss_dict, epoch_times = train(model, int(0), train_loader, val_loader, len(train_ds), len(val_ds), lr, self.eigenLoss, epochs, eigenlossHyper)
...
def getInitFunc(distributionName):
    ...
    elif distributionName == 'spikeAndSlabVeryLarge':
        return initLibrary.spikeAndSlabVeryLarge
    elif distributionName == 'spikeAndSlabLarge':
        return initLibrary.spikeAndSlabLarge
    elif distributionName == 'spikeAndSlabMedium':
        return initLibrary.spikeAndSlabMedium
    elif distributionName == 'spikeAndSlabSmall':
        return initLibrary.spikeAndSlabSmall
    elif distributionName == 'spikeAndSlabVerySmall':
        return initLibrary.spikeAndSlabVerySmall
...
if __name__ == "__main__":
    l = [
            # ('fluidVeryLarge', 'fluidLossVeryLarge5'),
            # ('fluidLarge', 'fluidLossLarge5'),
            # ('fluidMedium', 'fluidLossMedium5'),
            # ('fluidSmall', 'fluidLossSmall5'),
            # ('fluidVerySmall', 'fluidLossVerySmall5')
            # ('oceanVeryWide', 'oceanVeryWide5'),
            # ('oceanWide', 'oceanWide5'),
            # ('ocean', 'ocean5'),
            # ('oceanNarrow', 'oceanNarrow5'),
            # ('oceanVeryNarrow', 'oceanVeryNarrow5')
            # ('fluidVeryWide', 'fluidLossVeryWide5'),
            # ('fluidWide', 'fluidLossWide5'),
            # ('fluid', 'fluidLoss5'),
            # ('fluidNarrow', 'fluidLossNarrow5'),
            # ('fluidVeryNarrow', 'fluidLossVeryNarrow5')
            ('fluidKOLL', 'fluidLossKOLLR'),
            ('fluidKOL', 'fluidLossKOLR'),
            ('fluidKO', 'fluidLossKOMR'),
            ('fluidKOS', 'fluidLossKOSR'),
            ('fluidKOSS', 'fluidLossKOSSR')
            #('ocean', 'oceanChangingAlpha')
            # ('fluid', 'fluidChangingAlpha10')
            #('cyclone', 'cycloneNewInit')
            # ('pendulum0-200', 'trying_new_200'),
            #('duffing-100', 'duffingTrying'),
            # ('fp-100', 'duffingTrying3')
            #('pendulum0-100', 'pendulumOtherFixes')
    ...
    for (ds, saveName) in l:
        ...
        if ds.startswith('pendulum'):      
            expCol.loadRunRegime(f'/home/156/{USER}/kae-cyclones/src/testingRegimeInitPendFix.json')
            epochs = 75
        else:
            expCol.loadRunRegime(f'/home/156/{USER}/kae-cyclones/src/eigenLossAblationWidth.json')
            epochs = 30
        print(expCol.runRegime)
        expCol.run(epochs=epochs, numRuns=1)
