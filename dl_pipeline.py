...
def eval(model, val_ds, ds_length, steps=4, koopman=True, device=0):
    criterion = nn.MSELoss().to(device)
    model.eval()
    model.to(device)
    ...
    for i, data in enumerate(val_ds):
        ...
        out, out_back = model(cyclone_array[0].unsqueeze(0).to(device))
        
        for k in range(steps-1):
            if k == 0:
                loss = criterion(out[k], cyclone_array[k+1].unsqueeze(0).to(device))
            else:
                loss += criterion(out[k], cyclone_array[k+1].unsqueeze(0).to(device))
    ...
    print(sum(error)/len(val_ds))

    return sum(error)/len(val_ds)
